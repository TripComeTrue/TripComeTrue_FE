...
import { SearchSpotScrollAPI } from '@/apis/search';
...
const SearchSpotScroll = () => {
  ...
  const loadMoreData = async () => {
    try {
      ...
      const data = await SearchSpotScrollAPI(queryCity, currentPage + 1);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = await SearchSpotScrollAPI(queryCity, 0);
...
export default SearchSpotScroll;
